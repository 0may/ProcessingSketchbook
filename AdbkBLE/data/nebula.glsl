...
void main( void ) {
	...
	float rads = radians(0.1*time*3.15);
	...
	vec3 curr_color = (tc*0.9) + (tc2*0.7);
	
	
	//Let's draw some stars
	
	float scale = 0.0*sin(0.3 * time) + 5.0; // controls flight speed
	vec2 position2 = (((gl_FragCoord.xy / resolution) - 0.5) * scale);
	float gradient = 0.0;
	vec3 color = vec3(0.0);
	float fade = 0.0;
	float z = 0.0;
 	vec2 centered_coord = position2 - vec2(sin(time*0.1),sin(time*0.1)); // determines flight target coord
 	//vec2 centered_coord = position2 - 2.0*vec2(mouseX/resolution.x-0.5,0.5-mouseY/resolution.y);
	centered_coord = rotate(centered_coord, rads); // rotate 
	...
	gl_FragColor = vec4(curr_color + vec3(0.1), 1.0);
}